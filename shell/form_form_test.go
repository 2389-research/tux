...
func TestFormValues(t *testing.T) {
	nameField := NewInputField().WithID("name")
	roleField := NewSelect[string]().WithID("role").WithOptions(
		Option("Admin", "admin"),
	).WithDefault("admin")

	f := NewForm(nameField, roleField)
...
func TestFormNavigation(t *testing.T) {
	f := NewForm(
		NewInputField().WithID("a"),
		NewInputField().WithID("b"),
		NewInputField().WithID("c"),
	)
	f.Init()
...
func TestFormDelegatesKeyToField(t *testing.T) {
	f := NewForm(
		NewInputField().WithID("a").WithLabel("Test"),
	).WithTheme(theme.Get("dracula"))
	f.Init()
