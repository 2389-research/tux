// shell/form_integration_test.go
package shell
...
func TestFullFormWorkflow(t *testing.T) {
	...
	usernameField := NewInputField().WithID("username").WithLabel("Username").WithValidators(Required(), MinLength(3))
	...
	f := NewForm(usernameField, roleField, activeField).
...
func TestMultiPageForm(t *testing.T) {
	f := NewForm(
		...
		Group("Step 2",
			NewInputField().WithID("email").WithLabel("Email"),
		),
		Group("Confirm",
